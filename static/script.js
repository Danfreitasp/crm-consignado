    ...
    function atualizarResumoOfertas() {
        ...
        ofertasPortRefin.forEach((oferta, indice) => {
            ...
            [
                ...
                brl(oferta.saldo),
                `${oferta.prazoContrato || '-'} total · ${oferta.parcelasPagas || 0} pagas · ${oferta.parcelasRestantes ?? '-'} faltam`,
                oferta.tabela,
    ...
    function calcularPortRefin() {
        ...
        ofertaPortRefinAtual = !erros.length && troco >= 0 && modoAtual() === 'port_refin'
            ? {
                ...
                parcelasRestantes: Math.max(0, Math.trunc(Number(portFields.prazoContrato?.value || 0)) - Math.trunc(Number(portFields.parcelasPagas?.value || 0))),
